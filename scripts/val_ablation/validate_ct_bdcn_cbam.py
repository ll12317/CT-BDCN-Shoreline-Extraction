# -*- coding: utf-8 -*-
"""
Pure validation script for EdgeEnhancedSegNet_UNetBDCN with CBAM.
...
same directory as this script and make sure it exposes EdgeEnhancedSegNet_UNetBDCN.
"""
...
BEST_WEIGHT_PATH = OUT_ROOT / "cntrans+BDCN+CBAM best.pth"

PRED_DIR = OUT_ROOT / "cntrans+BDCN+CBAM val_preds"
CMP_DIR = OUT_ROOT / "cntrans+BDCN+CBAM val_compare"
EDGE_PRED_DIR = OUT_ROOT / "cntrans+BDCN+CBAM val_edgepreds"
METRICS_DIR = OUT_ROOT / "cntrans+BDCN+CBAM metrics"

for directory in (OUT_ROOT, PRED_DIR, CMP_DIR, EDGE_PRED_DIR, METRICS_DIR):
...
from edge_enhanced_segnet_unetbdcn_cbam import EdgeEnhancedSegNet_UNetBDCN
...
def evaluate_val(k_edge: int = 5, thr_seg: float = 0.5, thr_edge: float = 0.5) -> None:
    print("\n=== VAL EVAL: cntrans+BDCN+CBAM ===")
    ...
    pa, recall, precision, f1, iou = compute_metrics(
    ...
    print(f"PA_edge(seg)        = {pa_edge:.6f}")
    print(f"Recall_edge(seg)    = {recall_edge:.6f}")
    print(f"Precision_edge(seg) = {precision_edge:.6f}")
    print(f"F1_edge(seg)        = {f1_edge:.6f}")
    print(f"IoU_edge(seg)       = {iou_edge:.6f}")

    print("\n--- Edge-head metrics against the edge-band target ---")
    print(f"PA_head(edge)        = {pa_head:.6f}")
    print(f"Recall_head(edge)    = {recall_head:.6f}")
    print(f"Precision_head(edge) = {precision_head:.6f}")
    print(f"F1_head(edge)        = {f1_head:.6f}")
    print(f"IoU_head(edge)       = {iou_head:.6f}")

    metrics_txt = METRICS_DIR / "cntrans+BDCN+CBAM_metrics_val.txt"
    ...
    print("  compare :", CMP_DIR)
    print("  edgepred:", EDGE_PRED_DIR)


if __name__ == "__main__":
    evaluate_val(k_edge=5, thr_seg=0.5, thr_edge=0.5)
