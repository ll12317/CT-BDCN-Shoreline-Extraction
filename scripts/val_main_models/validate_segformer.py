# -*- coding: utf-8 -*-
"""
Pure validation script for SegFormerCoast.

This script loads validation data, loads the trained SegFormer checkpoint, saves
predictions/comparison panels, and computes full-image and edge-band metrics.
The network definition is not included here and is imported from segformer.py.
"""
...
BEST_WEIGHT_PATH = RESULT_DIR / "segformer_best.pth"

PRED_DIR = RESULT_DIR / "segformer_val_preds"
CMP_DIR = RESULT_DIR / "segformer_val_compare"
METRICS_TXT = RESULT_DIR / "segformer_metrics_val.txt"
...
try:
    from segformer import SegFormerCoast
except ImportError as exc:
    raise ImportError(
        "Cannot import SegFormerCoast. Please put segformer.py in the same directory as this validation script."
    ) from exc
...
# ================== Model builder ==================
def build_model():
    """Build the same SegFormerCoast architecture used during training."""
...
def evaluate():
    ...
    print("\n=== SegFormer segmentation metrics on validation set ===")
    print(f"PA        = {pa:.4f}")
